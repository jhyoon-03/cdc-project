...
public class SLList {
    ...
    public void add(int index, int x) {
        IntListNode a = sentinel.next; // {0,1,2,3}
        if (size == 0) {
            sentinel.next = new IntListNode(x, sentinel);
        } else if (index == 0) {
            IntListNode b = new IntListNode(x, sentinel.next);
            sentinel.next = b;
        } else if (index >= size) {
            int i = size;
            while (i > 0) {
    ...
    public IntListNode reverseHelper(IntListNode a) {
        IntListNode temp = a;
        ...
        IntListNode tail = sentinel.next;
        tail.next = sentinel;
        return temp;
    }
